...
def network_diagnostic():
    ...
    try:
        ...
        try:
            s.bind(('192.168.100.100', 168))
            print("✅ Can bind to 192.168.100.100:168")
            local_ip = '192.168.100.100'
        except:
            print("⚠️ Cannot bind to 192.168.100.100:168, trying all interfaces...")
            s.bind(('0.0.0.0', 168))
            local_ip = '0.0.0.0'
            print(f"✅ Bound to all interfaces on port 168")
...
def cbFun(snmpEngine, stateRef, contextEngineId, contextName, varBinds, cbCtx=None):
    ...
    try:
        ...
        path = f"bproltdata/bprpon{pon_main}onu{onu}"
...
if __name__ == "__main__":
    ...
    # Configure transport
    print(f"\n🎧 Configuring SNMP trap receiver on {bind_ip}:168")
    
    config.addTransport(
        snmpEngine,
        udp.domainName,
        udp.UdpTransport().openServerMode((bind_ip, 168))
    )

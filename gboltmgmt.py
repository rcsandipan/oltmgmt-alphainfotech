...
def network_diagnostic():
    ...
    try:
        ...
        try:
            s.bind(('192.168.100.100', 163))
            print("✅ Can bind to 192.168.100.100:163")
            local_ip = '192.168.100.100'
        except:
            print("⚠️ Cannot bind to 192.168.100.100:163, trying all interfaces...")
            s.bind(('0.0.0.0', 163))
            local_ip = '0.0.0.0'
            print(f"✅ Bound to all interfaces on port 163")
...
def cbFun(snmpEngine, stateRef, contextEngineId, contextName, varBinds, cbCtx=None):
    ...
    try:
        ...
        path = f"gboltdata/gbpon{pon_main}onu{onu}"
...
if __name__ == "__main__":
    ...
    # Configure transport
    print(f"\n🎧 Configuring SNMP trap receiver on {bind_ip}:163")
    
    config.addTransport(
        snmpEngine,
        udp.domainName,
        udp.UdpTransport().openServerMode((bind_ip, 163))
    )

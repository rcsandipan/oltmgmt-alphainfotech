...
def network_diagnostic():
    ...
    try:
        ...
        try:
            s.bind(('192.168.100.100', 167))
            print("✅ Can bind to 192.168.100.100:167")
            local_ip = '192.168.100.100'
        except:
            print("⚠️ Cannot bind to 192.168.100.100:167, trying all interfaces...")
            s.bind(('0.0.0.0', 167))
            local_ip = '0.0.0.0'
            print(f"✅ Bound to all interfaces on port 167")
...
def cbFun(snmpEngine, stateRef, contextEngineId, contextName, varBinds, cbCtx=None):
    ...
    try:
        ...
        path = f"aptoltdata/aptpon{pon_main}onu{onu}"
...
if __name__ == "__main__":
    ...
    # Configure transport
    print(f"\n🎧 Configuring SNMP trap receiver on {bind_ip}:167")
    
    config.addTransport(
        snmpEngine,
        udp.domainName,
        udp.UdpTransport().openServerMode((bind_ip, 167))
    )
